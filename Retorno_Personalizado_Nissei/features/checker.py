...
def verificar_tarefas_datalog(
    ...
    if resultados:
        if len(resultados) >= 2000:
            ...
            corpo = (
                f"Rotina de monitoramento Inventti x Nissei ({len(resultados)} Retornos travados)\n\n"
                "Segue planilha em anexo com registros dos retornos que estão travados em nossa aplicação:\n"
                + "Obs.: Só será alertado caso o total de retornos ultrapasse 2000 pendências.\n\n"
                + "Este e-mail é gerado automaticamente, favor não responder."
            )
            assunto = f"AVISO - RETORNO PERSONALIZADO NISSEI {datetime.now()}"
            ...
            print(f'Não há documentos suficientes pendentes {datetime.now()}')
